/**
 * 内置眼球资产：用 Three.js 基础几何和 Canvas 纹理生成示意模型，不读取 GLB 文件。
 * createEye() 在浏览器中调用；返回值与 import-model.js 的 loadModel() 使用同一结构。
 ...
 * entries=图文数据，procedural=true 标识内置模型，animations=[] 表示没有资产动画。
 */
export function createEye() {
  ...
  return { root, groups, materials, entries: anatomy, procedural: true, animations: [] };
}
